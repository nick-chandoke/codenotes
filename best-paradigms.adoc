...
* transform (change structure's shape)
  ** insert at arbitrary position
  ** add to or extend a side (concat, cons, snoc)
  ** rearrange (e.g. matrix transpose)
  ** delete
...
.structure symmetry
*every structure permits terse, elegant traversals & transforms when these functions are written in terms of the structure's symmetries.* reasoning by symmetry allows easily identifying solutions that would be very difficult to reason about by studying "frame-by-frame" updates to structures. non-coincidentally this is the same as reasoning about recursion: it's difficult to trace every function call, but much easier to understand in terms of closure and base & recursive cases.
