...
* i think i understand "better 100 fns on 1 data structure than 10 on 10." true, but even better is 30 fns on 1 structure. consider using lists for everything. now the empty element is `()`, like `false` in python or `0` in C. that's an element instead of a function, but it's the same principle. an example with functions is using `assoc` (lookup in alist) which returns `()` or the list where the first found element is. this is better than `find`, which returns `Maybe a`, since it also allows zero-cost traversal of the rest of the list. this makes parsers easy without lack of efficiency. again we test the object for truthiness instead of asking whether it's `Nothing` or `Just _`. also using lists for everything means that we need only some few functions, and we can use them for all kinds of traversals or constructions. of course, better than lists are zippers or trees, but still, these are implemented in terms of lists, so lists are inevitable, if even only as a low-level implementation. this being said, a good language would determine whether to use lists or trees depending on the constraints on each structure given by the programmer.
