...
how identifiers & context map to values. basically static vs dynamic and flat vs hierarchical: lexical (innermost context with identifier) or dynamic (one identifier stored in a global, stateful map). scope is an alternative to passing parameters (since passing parameters merely binds within the called function's scope.) wikipedia's article said that dynamic scoping globally binds within function bodies, then unbinds when the function returns. anyway scoping relates to: (de)allocation, context delimitation, semantics of free variables (usually illegal, but in lua and picolisp free vars are null. theorem provers (e.g. agda,) type checkers (e.g. haskell,) or logical deduction systems (e.g. datalog) could use them as part of a reïfication engine.)
...
===== dynamic scoping vs dynamic binding

dynamic binding is when expressions of free vars are valid; their value is determined at evaluation time. dynamic scoping is each identifier having a stack, and binding means pushing to that stack. wait...this definiton can implement lexical scoping; it's really just shadowing.

yeah, i'm giving up on trying to understand how binding and scope are different. for example, picolisp uses dynamic binding (ok) but supports lexical scoping via transient or private symbols. to me, that seems like lexical binding. it seems that the difference has something to do with whether a language is compiled vs interpreted. either way, it appears to be at least an unhelpful (if not invalid) distinction.
