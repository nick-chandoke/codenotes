...
there's no need for an ad-hoc map function; one cam combine symmetric and asymmetric mapping in `map`: `(map (λ (x) (let ([y (add1 x)]) (cond [(f y) ...] ...)) s)` shows y as being the result of applying a fn to each element, and the `cond` is the ad-hoc map.
...
rather than have a _checking_ system to help catch mistakes, it's better to have an _encoding_ system that fits together well. *checking without inference is cruft.*
...
* lisp is only as nice & flexible/dynamic as lispers say when the lisp does not discern between symbols and identifiers, and uses only alists as a scoping mechanism. no variables: only symbols. no structs: only alists. no scoping: only lists. pil does this; it does not distinguish between symbols and identifiers. however, there must still be a way to define items in the list in terms of other items bound in the same list (effectively `let*` & `letrec`); perhaps this means using vectors instead of lists. again, though, i _never_ have used `letrec`, so who really cares? either way, btw, we'd want our structure to be mutable since that's efficient and gives more options, and mutation is usually expected in a proglang, and we want to use data as an _alternative_ to language, so it must have the same capabilities, e.g. support of mutual recursion or efficient mutation of vars.
...
j, apl, burlesque, bqn. sql is array-based but not tacit.
