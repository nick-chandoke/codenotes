...
tl;dr: the "problem with state" is really multiple agent accessing a common state, where the state change is supposed to affect only a proper subset of those argents, but instead affects a superset of that subset. solutions are:
...
or conbinations thereof, e.g. rust's ownership model, which is basically a combination of semaphores and scope with parameter passing.
