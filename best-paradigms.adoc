...
this is the functional style. concatenative/stack gives the perfect solution: `p a b if* g` becomes `p [ t a ] [ f b ] if g h` where `h` is a binary function one of whose arguments tells which branch was taken. we still put `a` or `b` on the stack for `f`, but after `f ( x -- )` uses it for side effect, `t` or `f` remains atop the stack, and thus effectively becomes the whole segment of code's return value.
...
can be, in a stack lang, equally interpreted as `h(f(x),g(y))` [app] i.e. `x f y g h` [stack] or, assuming `h` as `if`, then `f` & `g` would be program branches. actually, this can already be done in any applicative language; it's just that in a stack lang managing variables across branches is easy b/c they're all just on the stack rather than needing to manage multiple names & scopes. this being said, mutating dynamically-bound variables could be just about as good or better, depending on how many things would be pushed onto the stack.

we're so wonderfully enabled by the fact that stack languages are basically functional mixed with mutative: all variables are on the stack and all functions share a common input and output: the stack. that means no need to name variables, and makes control flow extremely simple. what's amazing is that it actually works.

btw, the common refactoring pattern for applicative languages, `(f (+ 3 5))` --> `(let (x (+ 3 5)) (* (f x) (g x)))`, is done by `dup` in stack langs: `3 5 + dup \ f dip g *` (though it'd really be written using non-primitive combinator `bi`: `3 5 + f g bi *`.)
