...
* *satisfy algebraic design*; functions are the only elements and composition is the only operation on them. this allows us to see the program for its structure rather than purpose.
...
obviously if a problem begs a particular structure then that should be used. for everything else, what efficient, elegant structure can we assume that satisfies our current needs, and our future needs without much refactoring? or can the most appropriate structure be automatically chosen at compile time and automatically interfaced efficiently with other differently-typed automatically-chosen structures?

structures are ranked by their specifity (to a problem) and speed for a set of operations. common operations:
...
* insert
  ...
  ** at beginning
* traversal
* search (ordered or not)
* concatenation

the classes of solution structures are:

TODO: no structure should be considered a substructure of "graph;" graph may be considered as an abstract object, but for data structures, it's much more sensible to identify cons as the primitive relation operator, then consider appropriate generation & traversal rules for your needs; the data structure follows.

[NOTE]
for efficiency, all structures should be built & consumed non-strictly; then `consume ∘ produce` allocates no memory. to implement non-strict in an otherwise strict runtime, use _sequences_: functions from index to element. notable index types are `0`, `Int`, `(Array Int)`.
...
implemenations of ads := [a...z] -> [b - a, ..., y - x]:
...
;; i couldn't find a way to do it with fold

(define-syntax-rule (| a is ...) (array-slice-ref a (list is ...)))
(define (ads a) (array- (| a (:: 1 #f 1)) (| a (:: 0 (sub1 (array-size A)) 1))))
----
