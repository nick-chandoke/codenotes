...
select x,x*10 from x join x using (x);
┌───┬──────┐
│ x │ x*10 │
├───┼──────┤
│ 1 │ 10   │
...
│ 3 │ 30   │
└───┴──────┘
update x set x =         (select x*10 as fx from x join x using (x)); -- wrong: sets all in x to 10
update x set x = fx from (select x as id, x*10 as fx from x join x using (x)) where x=id; -- correctly sets each x to f(x)
----
...
`update from` is a non-standard yet commonly supported form. instead of `update t set a = expr` we do `update t set a = expr from t2`. were our sql engine (sqlite) not support `update from`, we'd need to execute an `update` statement for each row in a table e.g.
