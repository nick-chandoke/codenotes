...
the stack is nice because programs leave items on the stack for subsequent programs. this is also done in sql, however; queries may mutate tables, obviously leaving their mutated versions available for use by subsequent queries. so how good are combinators compared to sql?
