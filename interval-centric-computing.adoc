...
intervals are far more ofted used than discussed. consider "2008, sometime after may". this refers to an unknown point/element of a space/set by an interval. it is thus has a hole to be filled i.e. a query to be answered, a variable whose value is to be identified. and being of a space, it supports traversals / query-answering strategies. _set_ is a primitive; it basically means "thing", with the understanding that things are comprised of other things, and that all things can be referred to collectively at once (as "all" or "everything", etc)—intrensic properties of reality. thus _set_ is the structure with the least possible information/definition. thus it hasn't information to make specific computations about it specialized/efficient, such as how traversals of binary search trees are more efficient & particular than equivalent traversals of general graphs (graph being a more general form of tree). in other words, sets are used ubiquitously (indeed, always) but not much can be said about them generally. however, _interval_ is also used ubiquitously, and supports very simple & efficient algebras suitable for many computations. intervals reflect order. order is essentially the one thing that data engineers use to make computation efficient: to divide & conquer, to reduce search spaces, to avoid traversals, etc. amazingly, however, no language has accounted for intervals in its model! interval, however, is not exactly what we programmers use; more properly, _traversals_ are ubiquitous: sequences of interval endomorphisms: with an initial value query space, search an associated interval (usually a proper subinterval i.e. a proper subset expressed more tersely in terms of intervals),..., evenually arriving at a terminal value. it's a seq instead of a set b/c we want to exploit order. thus in a nutshell i'm advocating for the consideration of sets as intervals; all values are expressible as numbers or sequences thereof, and thus they permit order. all sets of them can thus be expressed as sets of intervals, which supports an ordered, unique, canonical form. the algebra of this supports usual set operations intersection, union, difference, complement, but computationally efficient. it also supports lattice & space operations, lending itself to prolog-like query refinements. being a simple algebra, programs are algebraic and thus computable from queries, and can be considered interactively during development, yet easily support compliation from the mathematical model into a sequence of cpu instructions.
...
firstly, note how the array paradigm is special: it separates the operation from the data that it's applied to, at least insofar as its shape. this easily generalizes from "shape" to "data", and thus from applicative to tacit/pointfree. consider k the expression, `"A",/:("hi";"there";"guys")`; in factor, it's `{ "hi" "there" "guys" } [ CHAR: A prefix ] map`. factor's version makes the computation (the quotation) obvious, as well as how to express tacitly, since all factor code is tacit. agreeing with the reason why factor was so named, we can easily move that subprogram around into any sensible context—in this case, that's any, unless something will be prepended before the "A". separating data from operation is crucial b/c the operation is all the info of the program: it's the dom & cod, and the operation's properties e.g. commutativity, which is all needed to consider algebras of the operations/program. so really what i'm saying here is redundant: "only tacit code can be considered algebraically and virtually." the only new bit here is that arrays make shape tacit, which is a step toward total tacitness.
...
* `q group-by 0 [ + ] reduce` does not group then apply the fold (sum); it creates a hash table, then iterates over each element, computing its group id, then adding it to the corresponding value. the computation is more properly expressed as `q [ + ] <by-group>` where `: <by-group> ( q: ( x -- id ) r: ( a b -- c ) -- by-group )`. this "per-datum" style is much better than the "packing" style since, aside from inherently being better b/c it allows us to specify computation facts independently, often the packing isn't as we'd like so we must unpack or change the data structure. in j, `x u/.y` properly separates the grouping operation from the aggregate fn `u` by not even concerning the grouping operation; instead, `x` is the result of the operation. the only difference from my "per datum" version is that `y` is traversed twice instead of once.
...
AoS should be SoA. "inlining" structs (e.g. a 3-field struct corresponding to 3 contiguous data in a single array) works only if the fields share the same size, since that maintains alignment.
...
* perhaps, just as there should be the empty set, there should be, implicitly or otherwise, the ability to mean "match anything" i.e. an identity value for set intersection. perhaps not, though, just as the inclusion of the constant function or empty quotation is needed only to trivially satisfy a mandatory higher-order function parameter.
