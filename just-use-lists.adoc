...
the actual order system gets a bit more complicated from there, but this serves enough example. the actual version ended up being:

----
instrument selector -> ((instrument candles))
funds := float
pseudo-order := (quantity symbol type . deps) {- where type is 'moc, 'mkt, or a list of conditions -}
filled := (open time, open price)
positions := ((instrument money))
filled -> positions
positions, filled -> strategy(candles) -> pseudo-orders, portion determiners {- e.g. expected profit -}
loop (pesudo-orders, portion determiners -> pseudo-orders) over instruments -> pseudo-orders
pseudo-orders, candles -> open, filled
----

aside from readability, the the statements' order is meaningless. rather than assuming that there are objects with attributes, we do the opposite: we know that there are data, which we identify then group [relate] for simplification. there's no concept of "X has a Y;" instead, it's just "there're related things; how can we simplify description of thei relations?"

. `instrument selector` is an abstract variable; it's defined as whatever produces a list of lists of shape `(instrument candles)`
. there's a datum called `positions`, which is a list of lists of shape `(instrument money)`
. notice that `((instrument candles))` can be joined [sql] with `positions` on `instrument`
. there exists a morphism from strategy to portion determiners. there's no consideration of whether a strategy "contains" or "produces" determiners
. loop over the list of groups of `instrument`, `candles`, and `stategy`. within each iteration of the loop, the group is in scope.
. strategy is parameterized by / is a function of candles, and each strategy produces a list of pseudo-orders. thus each strategy is associated with both a list of pseudo-orders and a financial instrument
. given how `instrument` is related to candles, money via positions, and pseudo-orders via strategy, `instruments` is enough to have `pseudo-orders` and `portion determiners` in scope in the loop body. we know that `instruments` is a valid plural of `instrument` because `instrument` is ultimately within a list (precisely it's in a group `(instrument candles)`, of which there is a list)

notice that none of the arrows is named; we do not care about how in code the states are transmuted. we presume _nothing_ about which functions, subroutines, language features, or other programmatic devices shall be used to implement the arrow diagram. we name data (or groups thereof) but not arrows.
...
the most general loop form is `loop <arrow> until <cond>`.
