...
* *exploit*: row values; numbers & strings; [builtin] [non-]aggregate ("core") [window] fns; `join`, `select`.
  ** `coalesce` is short-circuiting `or` like in lisp. like `iif` it's just a more convenient representation of `case when`.
    *** `coalesce` returns the first non-null, or if all null, then returns `null`. it works with aggregates e.g. `select coalesce(sum(x),0) from t` returns scalar `0` for empty `t`. however, it does not work with window functions.
...
window fns are aggregate fns generalized to operate on overlapping subsets; compare with `group by`, which partitions into non-overlapping subsets.
...
a _window [frame]_ is a subset of a relation. the only thing that distinguishes it from a `select` expression is that each subset is associated with a row. this is much more powerful than:

. `join`, which associates each row being with exactly one other
. aggregate functions, which evaluate to a scalar, albeit one per group when `group by` is used.

therefore window functions are more powerful, but more particular, versions of aggregate fns which give aggregate outputs per row rather than just per group of rows.

* aggregate window functions don't present the bare columns problem.
* _per se_ window fns can be used as aggregates, and ordinary aggregates can be used `over` windows
* even non-aggregate window functions effectively implement stateful loops (folds). aggregate winfns implement stateful loops that would be a pain to implement with a fold because their state isn't easily expressed by a single accumulator value, as is the case for aggregate functions on a rolling window e.g. selecting a value and the sum of its immediate neighbors. this is easily defined by a for loop in c, but not so easily by `reduce` in factor (without lexical variables), and would be quite ugly by a fold in racket, haskell, python, &c.

NOTE: because winfns' results are not per-row, window functions cannot be aliased then used in `where` clauses.

as the sqlite docs show, the syntax is:

----
SELECT lag       (x)       FILTER(WHERE x>10) over     win1          WINDOW win1 AS (order by x)
        |         |                 |          |         |                                |
   window-fn (expr[,...])    [filter-clause]  OVER window-defn/name                  window-defn
----

the `window-defn` may feature `partition by` or `order by`. window fns are applied per partition. `partition` is semantically equivalent to `group by`, but is different syntax, particular to window fns. `order by` is interesting for window fns because unlike ordinary aggregates, *they're applied to sequences, more particular than sets. it's sql's single device for dealing with sequences.* for example, `row_number`'s functionality cannot be expressed in sql by any other means. *however*, window functions cannot be applied directly to other window functions; that can only be done indirectly by associating each group with a unique identifier then using `with`.

====== frame spec

* `range` & `groups` partition by element equality, where the elements are those of the `order by` expression(s).
* `unbounded preceding` & `unbounded following` just mean the min & max values of the subset [partition]
* in `BETWEEN a AND b`, either `a` may be `expr FOLLOWING` or `b` may be `expr PRECEEDING`. they cannot both be used simultaneously. for more complete description of rules, see the window function sqlite docs, _§2.2.2 frame boundaries_. the rules are sensible; they basically imply that the index referred to by `b` cannot be less than that which is referred to by `a`.
* notice, in the `frame-spec` syntax diagram, the redundancy of {`UNBOUNDED PRECEEDING`, `expr PRECEEDING`, `CURRENT ROW`}, how it's present within the `BETWEEN ... AND` syntax, too. the documentation notes that `...` is syntactic sugar for `BETWEEN ... AND CURRENT ROW`. `BETWEEN` allows you to effectively specify the window as a substring (commonly called a _slice_, such as by python or factor coders), though here it's specified in terms of `CURRENT ROW`. a more literal representation is a loop in C:

[source,c]
----
Vector out;
for(i=0;i<len;i++)
  out.push(winfn(slice(max(0,i-a),min(i+b,len))));
----

here `CURRENT ROW` corresponds to `i` and `a` & `b` represent the computed value of ``expr``'s as in `BETWEEN a AND b`.

the default `frame-spec`, `RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE NO OTHERS`, identifies all prefixes of the [ordered] partition. remember that `range` implies that partitions are done by the expressions of the `order by` clause.

* frame types specify the subsets that the aggregate will be applied to:
  ** `rows`: the subtring containing current row, whose boundaries are computed by `a` & `b`
  ** `groups`: all other values having `order by` values in common (equal to) with current row
  ** `range`: requires exactly one `order by` expression (call it `x`), but otherwise generalizes `groups`' grouping predicate from equality to being in an interval. `range` groups by all rows whose `x` are in a range; as usual, the range contains the current row's `x` (call it `cx`), and its boundaries are deterimend by `a` & `b`. for example, `range between 2 preceding and 3 following` identifies subsets of a relation `where x is between cx-2 and cx+3`.

exclusions (set subtractions):

`exclude no others`:: no filtering
`exclude current row`:: self-explanatory
`exclude group`:: useful for the `rows` frame type. exclude current row and its _peers_ (rows sharing the current's `order by` or `partition by` expression).
`exclude ties`:: exclude peers

`filter` filters-out rows from subsets before computing the window fn over that subset. the window fn is still computed for every row.

====== examples

.difference of adjacent elements

this example's essence is `lag`.

[source,sql]
----
create table x(x);
insert into x values(1),(10),(100);
with t(x,d) as (select x,lag(x) over (order by x) from x) select x-coalesce(d,0) from t;
----

returns a column of `1 9 90`.

.cdf
----
select row_number() over (order by x) as "index",x,cume_dist() over (order by x) as cdf from x;
┌───────┬──────┬────────────────────┐
│ index │  x   │        cdf         │
├───────┼──────┼────────────────────┤
│ 1     │ 1    │ 0.0909090909090909 │
│ 2     │ 3    │ 0.181818181818182  │
│ 3     │ 4    │ 0.272727272727273  │
│ 4     │ 10   │ 0.454545454545455  │
│ 5     │ 10   │ 0.454545454545455  │
│ 6     │ 12   │ 0.545454545454545  │
│ 7     │ 100  │ 0.636363636363636  │
│ 8     │ 150  │ 0.727272727272727  │
│ 9     │ 200  │ 0.818181818181818  │
│ 10    │ 1000 │ 0.909090909090909  │
│ 11    │ 2000 │ 1.0                │
└───────┴──────┴────────────────────┘
----
...
.median
----
with m(v,n) as (select x,ntile(2) over (order by x) from x) select max(v) as median from m where n=1;
┌────────┐
│ median │
├────────┤
│ 12     │
...
.moving average with period 4
----
select x,avg(x) over (order by x rows 3 preceding) as ma from x;
┌──────┬──────────────────┐
│  x   │        ma        │
├──────┼──────────────────┤
│ 1    │ 1.0              │
...
* `3 preceding` means 4 items per subset: the three preceding plus the current row.
* you may want to discard the first 4 rows, since they're not averages of the prior 4 elements; they're the averages of fewer than 4 elements. because window-computed values cannot be used in ``select``'s `filter` clause (nor in `having`, btw), we can use `with` to discard those rows: `with y(n,x,ma) as (select row_number() over w,x,avg(x) over w from x window w as (order by x rows 3 preceding)) select x,ma from y where n>=4`
...
this does not produce (1)++(2),(3)++(3),(4),(5)++.... `select` does not return the whole table on each iteration; as described in §3 of sqldocs for `with`, one item is taken from a queue (step 2a). `select` is somewhat a misnomer in recursive queries.
...
TODO: what's `ascii`? w/e it is, it risks writing control characters to terminals.
...
* to open a db as read-only, link:https://www.sqlite.org/uri.html[specify its location as a uri] then append a query: `file://<absolute path>?mode=ro`. this can still cause the "database is locked" error depending [somehow] on how the program which has opened the db for rw has done so.
  ** if reading e.g. firefox's `places.sqlite` (bookmarks & history) while firefox is open (and so has the db locked) then you can feasibly, given that the file is small (15M for me) copy it to a new location then open it there. obviously this hack is infeasable for large files or when such copying must occur too frequently, as this would either take too long or age the disk unduly.
...
remember that pragmas require a statement-terminating semicolon.
...
* if using sqlite as a library in another proglang, then conversion from rows to json is trivial; use a json library for that proglang
* json is stored as ordinary strings, except return value of `json`
* json is stored in table cells or string literals

.importing json string into relation

suppose that i've a file `f.json` whose contents is an array of objects with keys `date`, `symbol`.

.reading from process
[source,sql]
----
create table tmp(t); -- to hold the raw string that'll be parsed into json
-- NOTE: i do not know whether \037 or \036 are used to partition import string into tables or not when --ascii isn't used
.mode ascii
-- or .import "filepath.json"
.import "|curl 'https://xyz.com/a/json/endpoint'" tmp
-- avoid outputting to terminal with .mode ascii!
.mode box
create table t as select value,id,parent from json_each(json(t)) join tmp;
-- select dates after today that're T+1 or fewer days away
-- e.g. if today is the 11th then with T=1 it'll select symbols & dates
-- whose dates are the 12th or 13th.
with n(n) as (select ceiling(strftime('%J',CURRENT_TIMESTAMP)))
  select json_extract(value,'$.symbol') as symbol,
         json_extract(value,'$.date') as date
  from t,n
  where cast(strftime('%J',date) as integer) between n and n+T;
----

.reading from file

this method has 4 fewer lines and does not use pragmas, though it still uses special function `readfile` which is available only inside the sqlite repl.

in either case, if you were using the sqlite library, you'd simply pass a string in directly. i'm not aware of json ever being designed maliciously for sqlite's parser.

[source,sql]
----
create table t as select value,id,parent from json_each(json(readfile("f.json")));
with n(n) as (select ceiling(strftime('%J',CURRENT_TIMESTAMP)))
  select json_extract(value,'$.symbol') as symbol,
         json_extract(value,'$.date') as date
  from t,n
...
NOTE: if you set `.mode json` before you `.import` then it'll parse each JSON token as a row, like `json_tree` except without that fn's columns, instead with only one column whose value is a k/v pair or a delimiter for an array or json object. i don't see when this would be appropriate.

use sqlite repl function/pragma `.cd` if necessary.

.parse a json table into sql

this parses an array of uniform-schema objects into a table `t`, inferring the relation's header from the 1st element's keys.

[source,sql]
----
-- list so that we can easily copy & eval the sql statement output by our sql query
.mode list
-- array of json objects becomes a table of records
create table src(j); insert into src select value from json_each(json(readfile("actives.json")));
-- eval the sql statements output by the following query; they create & populate the table corresponding to the original json
with j(h) as (select j from src limit 1) select 'create table t('||group_concat(key,',')||'); insert into t select '||group_concat('json_extract(j,"$.'||key,'"),')||'") from src;' from json_each(h) join j;
drop table src;
.mode box
select * from t;
----
...
common, useful, non-trivial fn names are bolded.
...
| json_array(e,...)                     | constructor (not accessor)
| json_object(k,v,...)                  | constructor (not accessor)
| json_array_length                     | duh. useful in query predicates.
...
| *json_group_array, json_group_object* | aggregate fn. return selection as json array or object (see example below). the fns take 1 & 2 args respectively.
|======================================================================================================================================================================================================================

NOTE: there's no function to get headers from a transient table. to know the headers returned by json functions, you must read the sqlite documentation. namely we cannot `select * from json_[each|tree](...)` because this includes the hidden column `json`, which returns a huge text string.
...
TODO: why is there a section on triggers here? haven't i already discussed them in better detail elsewhere?
...
firstly, i imagine that every desired extension to sqlite can be accomplished via its c api. though c is compiled. sqlite is extensible and can dynamically load functionality (loads shared object), by:

. if you've used `sqlite3_db_config(<db>,SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION,1,NULL)` in C, then `load_extension(name[,path])` in some sql statement (e.g. `select`) and `sqlite3_load_extension()` both are equivalent ways to load an extension.
. pragma `.load <ext>` in the sqlite repl, which does not require enabling extension loading.
  .. `.load` takes a 2nd, optional arg which specifies the shared object's function name to load; the default is `sqlite3_extension_init` or `sqlite3_<x>_init` where `x` is derived from the filename

to compile extensions you need one of the amalgamation files, `sqlite3.h`, which is not included in the sqlite source archives. you have 3 options for obtaining it:

. install package `libsqlite3-dev` from a package manager that features it.
. download the sqlite amalgamation
. manually generate the amalgamation (see <https://www.sqlite.org/src/doc/trunk/README.md>)

see `getthecode` doc. `fossil clone https://www.sqlite.org/src sqlite.fossil` is probably the best way to obtain.
...
* `sqlite3` as a command-line interface to sqlite: `sqlite3 -csv <file> <sql>` will print in csv, which can be processed by awk or w/e else. `xargs` is really useful with such outputs for generating shell scripts from sqlite dbs
...
`.import --csv path/to/csv mytable` to import [read: `UNION ALL`] a csv with a table. the latter is the preferable alternative to `.load csv`, which both requires having a separate shared object file and knowing its path. somewhat unfortunately we cannot simply invoke `sqlite3` with a csv pathname. still, csvs are pretty nearly queryable as though they were proper sqlite dbs. actually, the best thing to do is use link:https://github.com/harelba/q[q] instead of sqlite directly here. csv files are easily editable using a text editor, yet q can query them like they're sqlite. this is an elegant & easy way to have modifiable but queryable dbs.

the following may also be helpful:

* link:https://github.com/simonw/csvs-to-sqlite[csvs-to-sqlite]
* link:https://github.com/thombashi/sqlitebiter[sqlitebiter]: convert many kindsa of data to sqlite
