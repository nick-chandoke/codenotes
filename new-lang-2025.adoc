...
the essential quality of arrays is that elements are considered independently of each other; for example, `map` acts on one element without regard to others. this is different if we use `reduce` or `clump-map` (as it's called in factor; in k it's called `':`). of course, the independent case is simpler than the non-independent one, so it's easier to express the former in any language than the latter. though array langs are much better for the former, they're not so much better than non-arrays langs for the latter. consider the following efficient version of sql `select distinct y from x join y where y>=x group by x` or k `(?x{*(~x>)#y}\:y)^0N`:
