...
organizer: task(loc desc start end freq priority timecost). item(desc havecnt wantcnt usefreq cost priority). traveling salesman on taxicab metric. i need the above predicate system to support at least a relation's attribute being the predicate any e.g. `x or y or z`.
