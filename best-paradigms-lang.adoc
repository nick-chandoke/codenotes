== best paradigms lang (substring relation notations)

this document is supposed to be a terse spec of the ideal programming language. it's changed greatly since discoveries about sql and the relationship among [text|byte]strings, numbers, sequences, and strings.
...
* regexes are denoted by a leading ρ; like in `sed`, the following character determines the delimiter e.g. `ρ-'-' is interpreted as a literal, not a regex in here because it's the delimiter-`
...
* any computation with a trailing `=` is shorthand for `x=expr(x,...)`, including indexing ops.
...
* numbers are interpreted as digitstrings; x[i]=1 strings the ith bit of x

.operations

we generalize gmatch, match, find, find all, sub, and gsub by seeing them as traversals that may exit early by some predicate of loop state vars. this reduces the 6 functions to match, find, and sub that return a variable-sized substrings. substitution vs extraction is determined by presence of `=`. to choose between match or find,...idk, b/c idk why/when one would want to return indices rather than values there.
...
* select: `[i]` `[i:j]`; generalizes substring & find by allowing `i` or `j` to be an index or predicate
  ...
  ** replacement is done by using `=` inside the clause; this is separate from `=` outside the clause which chooses whether we mutate a variable or not.
  ** generally we may select a predicated substring by a given index. the notation may be `x[v@I]` for index `I`. indexes tell how to traverse a string, and exist for efficiency. `[]` passes `v` as a value to `I`; for example, `I` may be binary search, whose traversal of `x` depends on `v` being an integer index of `x` by common numerical order and assumes that `x` is sorted.
...
you may have noticed that the above rules imply that strings are not of one index, though every string has a natural index. like in sql, we may define any indexes over any strings, and the index may be of any form [type]. also the language is truly not only plurality-agnostic, but information-agnostic; no string exactly identifies some information; only the combination of a string and parser indicates information.
